...
public sealed class PageNavigationRegistry : IPageNavigationRegistry
{
    ...
    public PageNavigationRegistry(ILogger<PageNavigationRegistry> logger)
        : this(logger, typeof(HomePage), NavigationKeys.Home)
    {
    }
    ...
    public IPageNavigationRegistry Register(string key, Type pageType)
    {
        ...
        if (_registrations.ContainsKey(key))
        {
            _logger.LogWarning("Overwriting existing registration for key {Key} from {OldPageType} to {NewPageType}",
                key, _registrations[key].Name, pageType.Name);
        }
    ...
    public IPageNavigationRegistry RegisterDefaultPages()
    {
        ...
        _ = Register(NavigationKeys.Home, typeof(HomePage));
        _ = Register(NavigationKeys.Characters, typeof(CharactersPage));
        _ = Register(NavigationKeys.Pets, typeof(PetsPage));
        _ = Register(NavigationKeys.Inventory, typeof(InventoryPage));
        _ = Register(NavigationKeys.Crafting, typeof(CraftingPage));
        _ = Register(NavigationKeys.Factions, typeof(FactionsPage));
        _ = Register(NavigationKeys.Donate, typeof(DonatePage));
