...
public sealed class FileValidator
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            decryptedData = _cipherService.Transform(encryptedData);
        }
        catch (Exception ex)
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            try
            {
                decryptedHeader = _cipherService.Transform(headerBuffer.AsSpan(0, bufferSize).ToArray());
            }
            catch (Exception ex)
            {
                return ValidationResultInfo.Failure(
            ...
            try
            {
                ...
                try
                {
                    decryptedFooter = _cipherService.Transform(footerBuffer.AsSpan(0, bufferSize).ToArray());
                }
                catch (Exception ex)
                {
                    return ValidationResultInfo.Failure(
